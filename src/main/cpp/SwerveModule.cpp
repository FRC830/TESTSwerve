...
SwerveModule::SwerveModule(const int driveMotorChannel,
    ...
    : m_driveMotor(driveMotorChannel, rev::CANSparkMax::MotorType::kBrushless),
      ...
      //Commented out as declared in header file
...
void SwerveModule::SetDesiredState( const frc::SwerveModuleState& referenceState) {
    ...
    const auto driveOutput = m_drivePIDController.Calculate(m_driveEncoder.GetVelocity(),
                                                            state.speed.value());

    const auto driveFeedforward = m_driveFeedforward.Calculate(state.speed);

    // Calculate the turning motor output from the turning PID controller.
    const auto turnOutput = m_turningPIDController.Calculate(units::radian_t(m_turningEncoder.GetPosition()),
                                                             state.angle.Radians());
